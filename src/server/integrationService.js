...
module.exports = class IntegrationService {
    ...
    getConferenceSessionDetails(req, res) {
        ...
        if (req.params.id) {
            ...
            soqlQuery = 'SELECT Id, Name, Room__c, Date_and_Time__c, Description__c FROM Session__c';
        }
        ...
        this._runSoql(conn, soqlQuery)
            .then((records) => {
                ...
                const formattedData = records.map((sessionRecord) => {
                    ...
                    let dateTime = new Date(sessionRecord.Date_and_Time__c);
                    let dateTimeFormatted = dateformat(dateTime, 'dddd, mmmm dS, yyyy, h:MM TT')
